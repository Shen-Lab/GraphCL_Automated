...
class PygGraphPropPredDataset(InMemoryDataset):
    ...
    def generator_generate(self, data, n_generator=1):
        with torch.no_grad():
            prob = self.generators[n_generator-1].generate(data)
            prob = prob.numpy()

        node_num = data.x.size()[0]
        idx_sub = np.random.choice(node_num, 4, replace=False).tolist()
        idx_neigh = idx_sub
        idx_edge = []
        ...
        edge_index, _ = tg_utils.subgraph(idx_sub, idx_edge, relabel_nodes=True, num_nodes=node_num)
        ...
        # node_depth
        node_depth = data.node_depth[idx_sub]
        subset = torch.tensor(list(set(node_depth[:, 0].numpy().tolist())))
        n_idx = torch.zeros(data.node_depth.max() + 1, dtype=torch.int64)
        n_idx[subset] = torch.arange(subset.size(0))
        node_depth = n_idx[node_depth]
        ...
        # node_dfs_order
        node_dfs_order = data.node_dfs_order[idx_sub]
        subset = torch.tensor(list(set(node_dfs_order[:, 0].numpy().tolist())))
        n_idx = torch.zeros(data.node_dfs_order.max() + 1, dtype=torch.int64)
        n_idx[subset] = torch.arange(subset.size(0))
        node_dfs_order = n_idx[node_dfs_order]
        ...
        data.node_depth = node_depth
        data.node_dfs_order = node_dfs_order
        data.node_is_attributed = data.node_is_attributed[idx_sub]
    ...
    def get(self, idx):
        ...
        # generative model
            ...
            if data.num_nodes <= 500: # otherwise OOM
                data1 = self.generator_generate(self.trans(data1), 1)
                data2 = self.generator_generate(self.trans(data2), 2)
            edge_index_neg = tg_utils.negative_sampling(data.edge_index, num_nodes=data.x.shape[0])
            data._edge_index = data.edge_index
            data.edge_index_neg = edge_index_neg

        return self.trans(data), self.trans(data1), self.trans(data2)


from torch_geometric.data import Batch
def collate(data_list):
    batch = Batch.from_data_list([data[0] for data in data_list], follow_batch=['_edge_index', 'edge_index_neg'])
