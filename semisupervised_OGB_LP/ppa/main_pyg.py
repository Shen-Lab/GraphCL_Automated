...
def main():
    ...
    if not args.n_splits == 0:
        from sklearn.model_selection import StratifiedKFold
        skf = StratifiedKFold(n_splits=args.n_splits, shuffle=True)
        y = dataset.data.y[split_idx["train"]].numpy()
        for _, idx in skf.split(split_idx["train"], y):
            break
    ...
    if not args.pretrain == '':
        checkpoint = torch.load(args.pretrain_weight + '/checkpoint_100.pt')
