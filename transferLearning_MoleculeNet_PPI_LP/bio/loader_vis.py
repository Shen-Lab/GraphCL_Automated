...
class BioDataset_graphcl(BioDataset):
    ...
    def generator_generate(self, data, n_generator=1):
        ...
        node_num, _ = data.x.size()
        idx_sub = np.random.choice(node_num, 10, replace=False).tolist()
        idx_neigh = idx_sub
        idx_edge = []
        # weighted random walk
        for _ in range(80): # limit the walk within 10 steps
            _idx_neigh = []
    ...
    def get(self, idx):
        data, data1, data2, data3, data4, data5 = Data(), Data(), Data(), Data(), Data(), Data()
        for key in self.data.keys:
            item, slices = self.data[key], self.slices[key]
            s = list(repeat(slice(None), item.dim()))
            s[data.__cat_dim__(key, item)] = slice(slices[idx],
                                                    slices[idx + 1])
            data[key], data1[key], data2[key], data3[key], data4[key], data5[key] = item[s], item[s], item[s], item[s], item[s], item[s]
        ...
        # generative model
            ...
            data3 = self.augmentations[0](data3)
            data4 = self.augmentations[1](data4)
            data5 = self.augmentations[2](data5)
...
def custom_collate(data_list):
    ...
    batch_3 = Batch.from_data_list([d[3] for d in data_list])
    batch_4 = Batch.from_data_list([d[4] for d in data_list])
    batch_5 = Batch.from_data_list([d[5] for d in data_list])
