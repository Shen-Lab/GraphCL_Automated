...
def train(args, loader, model_cl, optimizer_cl, model_1, optimizer_1, model_2, optimizer_2, model_ib, optimizer_ib, gamma, device):
...
def main():
    ...
    if args.resume == 0:
        torch.save({'graphcl':model.state_dict(), 'graphcl_opt': optimizer.state_dict(), 'generator_1':model_generative_1.state_dict(), 'generator_1_opt':optimizer_generative_1.state_dict(), 'generator_2':model_generative_2.state_dict(), 'generator_2_opt':optimizer_generative_2.state_dict()}, './weights_generative/checkpoint_ibgamma'+str(args.gamma)+'_0.pth')
    else:
        checkpoint = torch.load('./weights_generative/checkpoint_ib_'+str(args.resume)+'.pth')
    ...
    for epoch in range(args.resume+1, args.epochs+1):
        ...
        pretrain_loss, generative_loss, link_loss = train(args, loader, model, optimizer, model_generative_1, optimizer_generative_1, model_generative_2, optimizer_generative_2, model_ib, optimizer_ib, gamma, device)
