...
import experiment_generative, experiment_generative_ibalone, experiment_generative_ib
import experiment_generative_linkPrediction
import pdb
...
parser.add_argument('--gamma_ib', type=float, default=0.1)
parser.add_argument('--principle', type=str, default='infomin')
...
def get_model_with_default_configs(model_name,
    ...
    if model_name == 'ResGCN_graphcl' and (not args.aug_mode == 'generative'):
        def foo(dataset):
    ...
    elif model_name == 'ResGCN_graphcl' and args.aug_mode == 'generative':
        def foo(dataset):
            return ResGCN_graphcl(dataset=dataset, hidden=hidden, num_feat_layers=num_feat_layers, num_conv_layers=num_conv_layers, num_fc_layers=num_fc_layers, gfn=False, collapse=False, residual=residual, res_branch=res_branch, global_pool=global_pool, dropout=dropout, edge_norm=edge_norm), \
                   vgae_encoder(dataset=dataset, hidden=hidden, num_feat_layers=num_feat_layers, num_conv_layers=num_conv_layers, num_fc_layers=num_fc_layers, gfn=False, collapse=False, residual=residual, res_branch=res_branch, global_pool=global_pool, dropout=dropout, edge_norm=edge_norm), \
                   vgae_decoder()
...
def run_experiment_graphcl(dataset_feat_net_triple
    ...
    if args.principle == 'infomax':
        experiment_generative.experiment(dataset, model_func, epochs=args.epochs, batch_size=args.batch_size, lr=args.lr, weight_decay=0, dataset_name=dataset_name, aug_mode=args.aug_mode, aug_ratio=args.aug_ratio, suffix=args.suffix)
    elif args.principle == 'infobn':
        experiment_generative_ibalone.experiment(dataset, model_func, epochs=args.epochs, batch_size=args.batch_size, lr=args.lr, weight_decay=0, dataset_name=dataset_name, aug_mode=args.aug_mode, aug_ratio=args.aug_ratio, suffix=args.suffix)
    elif args.principle == 'infominbn':
        experiment_generative_ib.experiment(dataset, model_func, epochs=args.epochs, batch_size=args.batch_size, lr=args.lr, weight_decay=0, dataset_name=dataset_name, aug_mode=args.aug_mode, aug_ratio=args.aug_ratio, suffix=args.suffix)
