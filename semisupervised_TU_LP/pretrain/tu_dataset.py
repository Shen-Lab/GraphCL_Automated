...
class TUDatasetExt(InMemoryDataset):
    ...
    def __init__(self,
        ...
        if name == 'COLLAB':
            self.n_walk, self.n_walkLen = 4, 10
        else:
            self.n_walk, self.n_walkLen = 16, 20
    ...
    def set_aug_mode(self, aug_mode='uniform'):
    ...
    def set_generator(self, generator_1, generator_2):
        self.generator = [generator_1, generator_2]

    def generator_generate(self, data, n_generator):
        with torch.no_grad():
            prob = self.generator[n_generator-1].generate(data).numpy()
        node_num, _ = data.x.size()

        try:
            idx_sub = np.random.choice(node_num, self.n_walk, replace=False).tolist()
        except:
            idx_sub = np.random.choice(node_num, self.n_walk, replace=True).tolist()
        idx_neigh = idx_sub
        idx_edge = []
        # weighted random walk
        for _ in range(self.n_walkLen): # limit the walk within 10 steps
            _idx_neigh = []
