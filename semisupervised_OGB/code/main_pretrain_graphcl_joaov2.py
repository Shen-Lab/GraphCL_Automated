...
def train(model, device, loader, optimizer, aug_P, gamma):
    ...
    n_proj = np.random.choice(25, 1, p=aug_P)[0]
    n1_proj, n2_proj = n_proj//5, n_proj%5
    ...
    gamma = gamma
    beta = 1
    b = aug_P + beta * (loss_aug - gamma * (aug_P - 1/25))
    ...
    aug_P /= aug_P.sum()
    print(loss_aug.reshape((5, 5)))
    print(aug_P.reshape((5, 5)))
...
def main():
    ...
    dataset = PygGraphPropPredDataset(name=args.dataset, mode='sampling')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), './weights/' + args.dataset + '_graphcl_joaov2_' + str(args.gamma) + '_' + str(epoch) + '.pt')
